...
class _JobDetailPageState extends State<JobDetailPage> {
  ...
  Future<void> _checkIfAlreadyApplied() async {
    ...
    final bool applied =
        await ApplicationService.instance.hasApplied(jobId: widget.job.id);
  ...
  Future<void> _handleApply() async {
    ...
    final ApplyResult result =
        await ApplicationService.instance.apply(jobId: widget.job.id);
    ...
    final String message = switch (result.status) {
      ...
      ApplyStatus.error        => result.message ?? 'Something went wrong. Please try again.',
    };
...
class _JobDetailPageLoaderState extends State<JobDetailPageLoader> {
  ...
  Future<void> _loadJob() async {
    try {
      final Job? job =
          await JobService.instance.fetchJobById(widget.jobId);
      if (!mounted) {
        return;
      }
      if (job == null) {
